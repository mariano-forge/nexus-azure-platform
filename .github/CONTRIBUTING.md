...
- Read the [README](README.md) for current project status — some parts of the roadmap are design-only (see "Beyond this repo"), not implemented yet.
- Check existing [ADRs](docs/adr/) — if your suggestion touches a decision already documented there, reference it.
...
1. Copy [`docs/adr/_template.md`](docs/adr/_template.md) to `docs/adr/NNN-short-description.md`.
2. Fill in Context, Decision, Rationale, Consequences, Alternatives Considered, and When to Reconsider.
3. Add it to [the ADR index](docs/adr/index.md).
4. Reference it in your PR.

This is the one rule that matters most here — the project's value is as much in the documented reasoning as in the code.

## Commit messages

[Conventional Commits](https://www.conventionalcommits.org/): `feat(terraform): add hub-spoke networking module`, `docs(adr): add ADR-009`, `fix(pipeline): correct Checkov config path`.
